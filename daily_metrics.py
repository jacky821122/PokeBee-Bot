...
def calculate_daily_metrics(target_date: str):
    ...
    first_peak_hour = top_hours.index[0] if len(top_hours) > 0 else 0
    first_peak_hour_dishes = top_hours.iloc[0] if len(top_hours) > 0 else 0
    first_peak_ratio = first_peak_hour_dishes / total_dishes if total_dishes else 0

    second_peak_hour = top_hours.index[1] if len(top_hours) > 1 else first_peak_hour
    second_peak_hour_dishes = top_hours.iloc[1] if len(top_hours) > 1 else 0
